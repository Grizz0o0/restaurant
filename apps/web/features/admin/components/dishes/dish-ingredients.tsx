...
export function DishIngredients({ dishId }: DishIngredientsProps) {
    ...
    let displayList: DisplayIngredient[] = [];
    if (!isModeSku) {
        displayList = baseIngredients.map((item) => ({
            ...item,
            isOverride: false,
            isSkuOnly: false,
    ...
    const handleStartEdit = (item: DisplayIngredient) => {
        setEditingId(item.inventoryId);
        setEditingQty(item.quantityUsed.toString());
    };

    const handleSaveEdit = (item: DisplayIngredient) => {
    ...
    const handleRemove = (item: DisplayIngredient) => {
        if (!isModeSku) {
            if (
                window.confirm(
    ...
    const availableInventoryItems = inventoryItems.filter(
        (item) => !displayList.some((i) => i.inventoryId === item.id),
    );
