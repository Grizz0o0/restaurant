...
import type { OrderType } from '@repo/schema';
...
export function PaymentModal({
    ...
    const handleConfirmPayment = async () => {
        try {
            const allOrders: OrderType[] = ordersData?.data || [];
            ...
            relevantOrders.forEach((order: OrderType) => {
                order.items?.forEach((item: OrderType['items'][0]) => {
            ...
            for (const orderId of orderIds) {
                const isDelivery = relevantOrders.find(
                    (o) => o.id === orderId,
                )?.addressId;
                if (isDelivery) {
                    console.log(
                        `Skipping COMPLETED status for delivery order ${orderId}`,
                    );
                    continue;
                }
            ...
            toast.error(`Thanh toán thất bại: ${(err as Error)?.message}`);
        }
    };
