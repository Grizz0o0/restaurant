...
function SkuSynchronizer({
    ...
    replaceSkus: (vals: NonNullable<DishFormValues['skus']>) => void;
    skuFields: Record<string, any>[];
}) {
    ...
    useEffect(() => {
        if (variants && variants.length > 0) {
            ...
            const currentKeys = JSON.stringify(
                currentSkus.map((s: { optionValues?: string[] }) =>
                    [...(s.optionValues || [])].slice().sort(),
                ),
            );
            const newKeys = JSON.stringify(
                newSkus.map((s: { optionValues?: string[] }) =>
                    [...(s.optionValues || [])].slice().sort(),
                ),
            );
...
export default function AdminDishesPage() {
    ...
    const languages = (languagesData as unknown as { data: { id: string; name: string }[] })?.data || [];
    ...
    const suppliers: { id: string; name: string }[] = suppliersData?.data ?? [];
    const categories = categoriesData?.data || [];
    const dishes = dishesData?.data || [];

    const [open, setOpen] = useState(false);
    const [editing, setEditing] = useState<DishDetailResType>(null);
    ...
    const openEdit = (dish: NonNullable<DishDetailResType>) => {
        // Intelligent Translation Selection
        const vi = dish.dishTranslations?.find(
            (t: { languageId: string }) => t.languageId === 'vi',
        );
        const en = dish.dishTranslations?.find(
            (t: { languageId: string }) => t.languageId === 'en',
        );
        ...
        form.reset({
            ...
            skus:
                dish.skus?.map((s) => ({
                    ...
                    stock: s.stock,
                    optionValues:
                        s.variantOptions?.map((o) => o.value) ?? [],
                })) ?? [],
        });
        setOpen(true);
    };
    ...
    const handleToggleActive = (dish: { id: string }) => {
        setDeleteId(dish.id);
    };
