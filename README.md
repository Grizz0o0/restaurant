...
- `apps/api`: REST API backend (NestJS) kết hợp kiến trúc tRPC native phục vụ API type-safe end-to-end. Chứa cả thư mục `prisma` để quản lý cơ sở dữ liệu (PostgreSQL).

### Packages (Shared)

- `packages/schema`: Chứa Zod schemas (`*.schema.ts`) dùng chung cho backend validation và frontend data types.
- `packages/trpc`: Chứa cấu hình root tRPC native (server/client types) để đồng bộ type safety end-to-end.
- `packages/constants`: Các hằng số, metadata phân quyền (`role.constant.ts`, v.v) dùng chung cho cả backend và frontend.
- Cấu hình khác: `packages/eslint-config`, `packages/typescript-config`.
...
- Rà soát end-to-end các module chính: auth, menu, order, payment, notification, contact, inventory, reservation.
